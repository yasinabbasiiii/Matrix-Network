...
# My statics
CONFIG_DIRECTORY='/etc/tinc/dam'
CONFIG_PATH='/etc/tinc/dam/tinc.conf'
HOST_DIRECTORY='/etc/tinc/dam/hosts'
...
install_core() {
    if ! command -v tincd &> /dev/null; then
        ...
        if command -v apt-get &> /dev/null; then
            colorize yellow "dam-Core is not installed. Installing...\n"
            apt-get update &> /dev/null
            apt-get install -y tinc  &> /dev/null
            colorize green "dam-Core installed successfully.\n"
            sleep 1
        else
...
configure_main_node(){
    ...
    read -p "	[*] Subnet (172.37.1.1): " subnet
    if [ -z "$subnet" ]; then
...
EOF
	...
	# Build tinc-up 
	show_progress 7 9 
	cat <<EOF > "/etc/tinc/dam/tinc-down"
#!/bin/bash
...
EOF
	...
	# Executable +x
	chmod +x /etc/tinc/dam/tinc-{up,down} &> /dev/null

	# Start Servies
	show_progress 8 9 
	systemctl enable tinc@dam &> /dev/null
	systemctl start tinc@dam &> /dev/null
...
add_new_node(){
    ...
    read -p "	[*] Subnet (172.37.1.x): " subnet
    if [ -z "$subnet" ]; then
...
mkdir -p /etc/tinc/dam/hosts/ &> /dev/null
sudo tee /etc/tinc/dam/tinc.conf &> /dev/null <<CONFIG
Name = $node_name
Device = $device
...
sudo tee /etc/tinc/dam/hosts/$node_name &> /dev/null <<NODE
Address = $host_ip
Subnet = $subnet
Port = $port
Cipher = $cipher
...
yes '' | tincd -n dam -K4096 &> /dev/null

echo -e "5. Configurating service files..."
sudo tee /etc/tinc/dam/tinc-up &> /dev/null <<UP
#!/bin/bash
/sbin/ip link set \\\$INTERFACE   up
/sbin/ip addr add $subnet dev \\\$INTERFACE  
/sbin/ip route add $route_subnet dev \\\$INTERFACE  
...
sudo tee /etc/tinc/dam/tinc-down &> /dev/null <<DOWN
#!/bin/bash
/sbin/ip route del $route_subnet dev \\\$INTERFACE 
/sbin/ip addr del $subnet dev \\\$INTERFACE  
/sbin/ip link set \\\$INTERFACE   down
...
#______________________ SSH CLOSED ____________________
	...
	#start dam service on node
	echo -e "9. Start dam serivce on the node server..."
	ssh -T $username@$host_ip "systemctl enable --now tinc@dam" &> /dev/null
	# Maybe service exists before so restart it
	ssh -T $username@$host_ip "systemctl restart tinc@dam" &> /dev/null
	...
	echo -e "11. Restart dam service on main server..."
	systemctl restart tinc@dam &> /dev/null
...
node_mangment_center(){
    ...
    colorize reset "	[2] Restart dam Service" | lolcat -S 30
    ...
    case $choice in
		...
		2) restart_dam_service $todo_node;;
...
change_port_number(){
	...
	if [[ $1 == "main" ]];then
	    ...
	    for node in "${HOST_DIRECTORY}"/*; do
        	...
        	if [[ $node_name != "main" ]]; then
        		...
        		ssh -o BatchMode=yes -T "$ip_address" "systemctl restart tinc@dam"  &> /dev/null
	...
	colorize green "	dam service restarted successfully" bold
	systemctl restart tinc@dam &> /dev/null
	echo ''
	press_key
}
...
remove_node(){
	echo ''
	echo -ne "	\033[0;33m\033[1mAre you to delete dam on $1 node? (yes/no)\033[0m\033[0m: "
    read answer 
	...
	if [[ "$1" == "main" ]]; then
		systemctl stop tinc@dam &> /dev/null
		systemctl disable tinc@dam &> /dev/null
		rm -rf "$CONFIG_DIRECTORY" &> /dev/null
		clear
		echo -e  "	\033[1mdam on $1 node deleted successfully\033[0m"  | boxes -d nuke | lolcat
		echo ''
		press_key
		return 0
...
restart_dam_service(){
	...
	if [[ "$1" == "main" ]]; then
		systemctl restart tinc@dam &> /dev/null
		colorize green "	dam service in $1 node restarted successfully \n" bold
		press_key
		return 0
	else
		local CONFIG_FILE=${HOST_DIRECTORY}/${1}
		local public_ip=$(grep -oP '(?<=Address = )\S+' "$CONFIG_FILE")
		ssh -T -o BatchMode=yes -o ConnectTimeout=3 "$public_ip" "systemctl restart tinc@dam" &> /dev/null
		colorize green "	dam service in $1 node restarted successfully \n" bold
		press_key
		return 0
	fi
...
check_service_status(){
    if  systemctl is-active --quiet "tinc@dam"; then
        echo -e "[﹡]    dam service status: Running            [﹡]" 
    else
        echo -e "[﹡]    dam service status: Stopped            [﹡]" 
    fi
}
...
display_menu() {
    ...
    echo -e '[﹡]    Github: github.com/Musixal/dam-network [﹡]' 
	check_service_status
    echo "	══════════════════════════════════════" | lolcat  
    echo ''
