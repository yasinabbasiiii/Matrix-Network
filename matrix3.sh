...
configure_main_node(){
    ...
    echo ''	
 	...
 	if $expert; then
 		echo -ne "	[-] Address family (ipv4|ipv6|${GREEN}any${NC}): "
 		read -r add_family
    ...
    # Add ConnectTo entries for each node
    for node in "${NODES[@]}"; do
        if [[ $node != "$node_name" ]]; then
            echo "ConnectTo = $node" >> $CONFIG_PATH
        fi
    done
...
add_new_node(){
    ...
    echo 
    echo "======================================================="
    echo -e " \e[32mMain > Add New Node \e[0m"
    echo "======================================================="
    echo

    # Prompt for new node details
    read -p "Enter the name of the new node: " node_name
    read -p "Enter the IP address of the new node: " host_ip
    read -p "Enter the username to connect to the new node: " username
    ...
    # Transfer SSH key to the new node
    sshpass -p $password ssh-copy-id -i $SSH_KEY $username@$host_ip

    # Create tinc configuration on the new node
    sshpass -p $password ssh $username@$host_ip "mkdir -p /etc/tinc/matrix/hosts"

    # Write tinc.conf on the new node
    sshpass -p $password ssh $username@$host_ip "echo 'Name = $node_name' > /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'Device = /dev/net/tun' >> /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'AddressFamily = any' >> /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'DirectOnly = yes' >> /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'AutoConnect = yes' >> /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'PingInterval  = 10' >> /etc/tinc/matrix/tinc.conf"
    sshpass -p $password ssh $username@$host_ip "echo 'ProcessPriority  = high' >> /etc/tinc/matrix/tinc.conf"

    # Add ConnectTo entries for each node in the new node's tinc.conf
    for node in "${NODES[@]}"; do
        if [[ $node != "$node_name" ]]; then
            sshpass -p $password ssh $username@$host_ip "echo 'ConnectTo = $node' >> /etc/tinc/matrix/tinc.conf"
        fi
    done

    # Transfer the main node's public key to the new node's hosts directory
    sshpass -p $password scp $HOST_DIRECTORY/$node_name $username@$host_ip:/etc/tinc/matrix/hosts/

    # Transfer the new node's public key back to the main node's hosts directory
    sshpass -p $password scp $username@$host_ip:/etc/tinc/matrix/hosts/$node_name $HOST_DIRECTORY/

    # Restart tinc on the new node
    sshpass -p $password ssh $username@$host_ip "systemctl restart tinc@matrix"

    echo
    echo -e "\e[32mNew node $node_name added and configured successfully.\e[0m"
    press_key
}
...
display_menu() {
    ...
    echo -e '[﹡]    Developer: Yasin                                 [﹡]' 
    echo -e '[﹡]    Github: github.com/yasinabbasiiii/matrix-network [﹡]' 
	check_service_status
    echo "	══════════════════════════════════════" | lolcat  
    echo ''
